...
Copilotに「このプロジェクトのルール」を覚えさせる仕組みを知る。指示書ファイルをリポジトリに置けばチーム全員（クラス全員）に同じ設定が配られることを体験する。
...
- Copilotはデフォルトではプロジェクトのルールを知らない → 指示書で教える、という発想を説明
- `.github/copilot-instructions.md` を自分のリポジトリに作る（例: 「このプロジェクトはTypeScriptを使う」「コメントは日本語で」など）
- 効果を確認: 同じ質問を指示書の有無で比べてみる
- Agent Skillsの紹介: 授業で使ってきた「AI導入用プロンプト」を `.github/skills/` のSKILL.mdにすると、貼り付け不要の自動発動に進化することをデモ

## 教えるポイント

- 「毎回同じことをAIに説明しているなら、それはファイルに書いてリポジトリに置く」——lint設定ファイルの共有（単位2）と同じ思想だと接続する
...
- 公式以外は品質がまちまち・基本英語。**配る前に講師がSKILL.mdの中身を読んで選ぶ**
- デモ案: 同じブログ画面を `apple-design` と Material Design 系スキルでそれぞれレビューさせ、デザイン思想の違いを見せる（単位6のデザインシステム比較とつながる）
...
**導入後の授業デモ案**: スキル導入の前後で「ブログのトップページのデザインを提案して」と同じ指示を出し、生成されるデザインを見比べる——「Inter・紫グラデ・角丸カード」の定番から抜け出せているかを全員で確認すると、スキルの効果が一目でわかる。
